	...
	def create(self):
		...
		for i in range(12):
			...
			if i==month or (i+12)==month:
				...
				self.canvas.create_text(self.convert.change(x_coord,y_coord-2*self.circle_size),fill=self.text_color,text=' '.join([str(date),month_name,str(year)]),font=self.font_analog)
			else:
