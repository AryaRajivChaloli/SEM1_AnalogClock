...
class make_clock1:
	def __init__(self,root,time_gap=0,w=500,h=650,check=0):
		self.def_world		= [-1,-1,1,1]
	...
	def create1(self):
		...
		for i in range(12):
			...
			if i==month or (i+12)==month:
				self.canvas.create_text(self.convert.change(x_coord,y_coord),fill='red',text=' '.join([str(date),month_name,str(year)]),font='Harrington 15')
			elif i==0:
				self.canvas.create_text(self.convert.change(x_coord,y_coord),fill='blue',text=str(12),font=self.font_digital)				
			else:
				self.canvas.create_text(self.convert.change(x_coord,y_coord),fill='blue',text=str(i),font=self.font_digital)
		self.create_needles()
...
class make_clock2:
	...
	def create1(self):
		...
		for i in range(12):
			...
			x_coord,y_coord	= cos(angle),sin(angle)
			if i==0:
				self.canvas.create_text(self.convert.change(x_coord,y_coord),fill='blue',text=str(12),font="Harrington 12")				
			else:
				self.canvas.create_text(self.convert.change(x_coord,y_coord),fill='blue',text=str(i),font="Harrington 12")
		#self.create_needles()
		self.create_circle(0,0)
	def create_circle(self,x,y,ismonth=False):
		radius			= self.circle_size/2.0
	...
	def create_needles(self):
		...
		draw_line(self.convert.coords(0,0,x_min,y_min),fill=self.needle_color,width=self.width_min)
		draw_line(self.convert.coords(0,0,x_sec,y_sec),fill=self.needle_color,arrow='last')
		self.canvas.create_text(self.convert.change(0,-1.4),fill="white",text="      "+digital_time,font="Harrington 12")
...
w,h		= 300,200
root		= Tk()
make_clock1(root,time_gap,500,200)
make_clock2(root,time_gap,180,500)
root.mainloop()