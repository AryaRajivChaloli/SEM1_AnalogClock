...
class make_clock:
	...
	def create(self):
		...
		self.Create_Month_Dial()
		self.Create_Date_Dial()
		self.Create_Day_Dial()
		for i in range(12):
			angle		= start-(i*step)
	...
	def create_circle(self,x,y,ismonth=False,radius=0):
		if not radius:
	...
	def Create_Month_Dial(self):
		...
		self.canvas.create_line(self.convert.coords(0,0.4,x_month,y_month+0.4),fill="red",width=3)
		self.canvas.create_text(self.convert.change(0,0.45),fill='white',text='Month',font='Papyrus 10')

	


	def Create_Date_Dial(self):
		year,month,date,*extra = datetime.timetuple(datetime.utcnow()-self.time_gap)
		...
		self.canvas.create_line(self.convert.coords(0,-0.5,x_date,y_date-0.5),fill="red",width=3)
	...
	def Create_Day_Dial(self):
		...
		self.canvas.create_line(self.convert.coords(-0.5,0,x_day-0.5,y_day),fill="red",width=3)
