...
class make_clock:
	def __init__(self,root,time_gap=0,w=500,h=500):
		self.def_world		= [-1,-1,1,1]
		self.bg_color_analog	= '#000000'
		self.bg_color_digital	= 'grey'
		self.font_digital	= 'Harrington 33'
		self.align_digital	= 'center'
		self.needle_color	= '#ffffff'
		self.month_color	= 'red'
		self.circle_color	= '#808080'
	...
	def create(self):
		extra,month,*extra = datetime.timetuple(datetime.utcnow()-self.time_gap)
	...
	def create_needles(self):
		self.canvas.delete(self.needles)
		self.label.destroy()
		...
		month_list		= ['January','Febrruary','March','April','May','June','July','August','September','October','November','December']
		...
		draw_line(self.convert.coords(0,0,x_hrs,y_hrs),fill=self.needle_color,tag=self.needles,width=self.width_hrs)
		draw_line(self.convert.coords(0,0,x_min,y_min),fill=self.needle_color,tag=self.needles,width=self.width_min)
		draw_line(self.convert.coords(0,0,x_sec,y_sec),fill=self.needle_color,tag=self.needles,arrow='last')
		self.label		= Label(self.frame,text=digital_time,background=self.bg_color_digital,font=self.font_digital,anchor=self.align_digital)
